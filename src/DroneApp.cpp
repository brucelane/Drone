...
class DroneApp : public AppNative {
public:
...
void DroneApp::prepareSettings(Settings* settings)
{
...
void DroneApp::keyDown(KeyEvent event)
{
	if (event.getChar() == 'e')
		mTimeInc *= 0.95f;
...
void DroneApp::update()
{
	...
	mCamera.setOrientation(orientation * ci::Quatf(Vec3f(0, 1, 0), M_PI));
...
CINDER_APP_NATIVE(DroneApp, RendererGl)
